...
const (
	maxMessageSize = 512
	readDeadline = 60 * time.Second // 1 min
	writeWait = 10 * time.Second // 10 segundos
	pingPeriod = (readDeadline * 9) / 10  // 90% da read deadline
)
	...
	for {
		...
		if err != nil {
			...
			c.Room.Broadcast <- Message{
				Kind: InvalidJSON,
				Message: "this messsage should be a valid json",
				UserID: m.UserID,
			}
			continue
		}
